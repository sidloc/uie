...
# Compute Features
    ...
    #compute masks
        ...
        # Masked RGB
        maskedDepth = data['rgb'][sampleIdx]* mask3
        hog_features=visualizeHOG(maskedDepth)
        ...
        #depSIFT = visualizeSIFT(maskedDepth)
        # Actual Features used
        #features=hog_features
        features = hog_features
        #features = np.concatenate((hog_features, seghog_features),axis=0)
        X.append(features)
    X=np.asarray(X)    
    return X
...
parameters = [
    {'kernel': ['rbf'], 'gamma': [1e-3, 1e-4], 'C': [10, 100,150]}
]
...
"""#compare random forest to svm
labels_test=labels_predicted
print("preTest Accuracy [%0.3f]" % ((labels_predicted == labels_test).mean()))
# Random forest
...
with open('Res_svmlincv5.csv', 'w', newline='') as csvfile:
